...
The full-stack application now connects the authenticated Raritone frontend to the existing FastAPI pose-analysis service.
...
Upload JPG / JPEG / PNG
      ↓
React validates type, size and dimensions
      ↓
...
YOLO pose model
      ↓
Pose + body measurements
      ↓
Express stores the authenticated user's result
      ↓
React result screen
```
...
The Express backend should point to it with:

```env
AI_SERVICE_URL=http://127.0.0.1:8000
```

### Try-On failure cases

The frontend and backend handle:

- Missing image
- Unsupported image type
- Image larger than 10 MB
- Invalid/corrupt image
- Image dimensions that are too small
...
Product details include a **Try On** action that sends the user to the authenticated Try-On Studio. The current milestone performs pose/body analysis. A future VTON provider can be added behind the same FastAPI boundary without exposing AI credentials to React.

### Security

AI service credentials and private keys must remain server-side. Do not place them in React environment variables or commit real `.env` files.
